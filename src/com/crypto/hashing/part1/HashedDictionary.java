...
public class HashedDictionary<K, V> implements DictionaryInterface<K, V>
{
   ...
   private static final double MAX_LOAD_FACTOR = 0.5; // Fraction of hash table
                                                      // that can be filled
   private static int probeType; 
   private int conflicts;		// The number of probing conflicts
...
public HashedDictionary()
   {
      this(DEFAULT_CAPACITY, probeType); // Call next constructor
   } // end default constructor
   
   public HashedDictionary(int initialCapacity, int probType)
   {
      initialCapacity = checkCapacity(initialCapacity);
      numberOfEntries = 0;    // Dictionary is empty
      this.probeType = probType;
      // Set up hash table:
...
//         System.out.print("\n\t Length "+hashTable.length + " Key " + key + " index " + index);
         
         index = probe(index, key); // Check for and resolve collision
//         System.out.println(" the resolved index is " + index);
	...
	{		
		if(probeType == 0) {
			...
			while ( !found && (hashTable[index] != null) )
			{
				...
				if (!found)		// Makes note of any conflicts that arise.
					conflicts++;
			} // end while			
		}
		else {
